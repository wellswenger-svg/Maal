...
`flux_clothed_figure_volume_v1.safetensors` ([Civitai 1802814](https://civitai.com/models/1802814/bigger-breasts-and-butts-flux-kontext-lora))  
...
| **Swap one Kontext bust LoRA into `breast_enhance`** | **Yes — try now** | [Bigger breasts and butts (Flux Kontext)](https://civitai.com/models/1802814) → `flux_clothed_figure_volume_v1.safetensors`, strength **0.55–0.70**. Replace Huge_natural — do not stack. |
...
1. Download `flux_clothed_figure_volume_v1.safetensors` onto the GPU Comfy `models/loras/`
...
|------------|------------------------------------------|------|--------------------|
| `breast_enhance` / `bust_enhance` | `flux_clothed_figure_volume_v1.safetensors` | Front volume (interim Kontext) | **0.55–0.70** |
| `ass_enhance` / `hip_enhance` | `flux_clothed_figure_hip_v1.safetensors` | Back / hips volume | **0.70** |
...
- Front: Civitai model `1802814` → `flux_clothed_figure_volume_v1.safetensors` (replaces Huge Natural Breasts `780114`)  
...
for f in ['flux_clothed_figure_volume_v1.safetensors','flux_clothed_figure_hip_v1.safetensors','aidmaNSFWunlock-FLUX-V0.2.safetensors','pulid_flux_v0.9.1.safetensors']:
...
| Current reshape weight (front) | `flux_clothed_figure_volume_v1.safetensors` (`breast_enhance`) @ 0.55–0.70 until custom keep-outfit LoRA ships |
| Current reshape weight (back) | `flux_clothed_figure_hip_v1.safetensors` (`ass_enhance` / `hip_enhance`) |
...
7. [ ] Near-term: install Kontext bust LoRA (replace Huge_natural) @ 0.55–0.70 — do **not** stack more Civitai bust LoRAs or PixAI
8. [ ] Gate holdout after swap; if still weak → gold / semi-gold → custom LoRA
