"""Iterate face-fluid gens on saree until gel is clearly visible (or LoRA fails).

Stores each attempt to Mongo (test_run, unopened) and writes local out.png.
"""
...
OUT = REPO / "tmp_test" / "18000_cumshot_iterate_nfa_strong"
IID = "6a8630fbbd8d7c30d36ddd6b"
PRESET = "cumshot_clothes"
BATCH = "18000_cumshot_iterate_nfa_strong"

# NFA only — COF v6 produced brown/glitch artifacts, not ref-like gel.
ATTEMPTS = (
    ("nfa_v3", "flux_facial_fluid_v1.safetensors", "1.20", 101),
    ("nfa_v4", "flux_facial_fluid_v1.safetensors", "1.25", 202),
    ("nfa_v5", "flux_facial_fluid_v1.safetensors", "1.30", 303),
)
...
async def _clear_comfy(settings) -> None:
    import json as _json
    import urllib.request

    url = settings.comfyui_url.rstrip("/")
    for path in ("/interrupt", "/queue"):
        try:
            if path == "/interrupt":
                req = urllib.request.Request(
                    url + path, data=b"{}", method="POST", headers={"Content-Type": "application/json"}
                )
                urllib.request.urlopen(req, timeout=10)
            else:
                with urllib.request.urlopen(url + path, timeout=10) as r:
                    d = _json.loads(r.read().decode())
                _log(f"queue running={len(d.get('queue_running') or [])} pending={len(d.get('queue_pending') or [])}")
        except Exception as exc:
            _log(f"comfy_{path}_err {exc}")
